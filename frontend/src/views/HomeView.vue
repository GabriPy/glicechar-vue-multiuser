<template>
  <div class="space-y-3">
    <!-- Alert Configurazione Mancante (No Gluroo Link) -->
    <div v-if="!auth.user?.gluroo?.link" 
      class="card bg-base-100 shadow-xl border-2 border-primary/30 overflow-hidden">
      <div class="card-body items-center text-center p-8 space-y-6">
        <div class="relative flex items-center justify-center">
          <div class="absolute w-20 h-20 bg-primary/10 rounded-3xl animate-pulse"></div>
          <div class="relative w-16 h-16 rounded-3xl bg-primary/20 flex items-center justify-center">
            <i class="fi fi-sr-cloud-share text-primary text-3xl leading-none"></i>
          </div>
        </div>
        
        <div class="space-y-3">
          <h2 class="text-2xl font-black uppercase tracking-tighter text-primary text-center w-full">
            {{ $t('home.no_config') }}
          </h2>
          <div class="space-y-1">
            <p class="text-sm font-bold opacity-80">
              {{ $t('home.no_config_desc') }}
            </p>
          </div>
        </div>
        
        <div class="card-actions pt-2">
          <router-link to="/settings" class="btn btn-primary btn-sm md:btn-md font-black uppercase tracking-widest px-10 shadow-lg shadow-primary/20">
            <i class="fi fi-sr-settings"></i>
            {{ $t('home.configure_now') }}
          </router-link>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="store.hasSyncError" 
